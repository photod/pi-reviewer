...
DEFAULT_FAMILIES="glm qwen minimax deepseek mimo kimi luna"
default_alias() {
  case "$1" in
    glm)      echo 'glm-5.2' ;;
...
default_tier() {
  case "$1" in
    ...
    med)  echo 'glm qwen deepseek' ;;
    high) echo 'glm qwen minimax deepseek mimo' ;;
    *)    echo '' ;;
  esac
}
...
cmd_show() {
  ...
  log "settings"
  local key val
  for key in tier chairman kimiMode; do
    val="$(json_py get "${key}")"
...
cmd_set() {
  local key="${1:-}" val="${2:-}"
  [[ -n "${key}" && -n "${val}" ]] || die "usage: pi-config.sh set <tier|chairman|kimiMode> <value>"
  case "${key}" in
    tier)
      ...
      ;;
    kimiMode)
      in_list "${val}" "opencode cli off" || die "invalid kimiMode '${val}' — use opencode, cli, or off"
      [[ "${val}" != "cli" ]] || command -v kimi >/dev/null 2>&1 || warn "kimiMode=cli but the 'kimi' CLI is not on PATH — the kimi leaf will come back UNAVAILABLE"
      ;;
    chairman)
      ...
      ;;
    *) die "unknown key '${key}' — settable keys are: tier, chairman, kimiMode (models → 'model', tiers → 'tier')" ;;
  esac
  json_py set "${key}" "${val}"
  log "set ${key} = ${val}"
}
...
cmd_tier() {
  ...
  [[ -n "${name}" ]] || die "usage: pi-config.sh tier <low|med|high> <family...> | --kimi on|off | --effort <e> | --unset"
  ...
  case "${1:-}" in
    ...
    --kimi)
      in_list "${2:-}" "on off" || die "usage: pi-config.sh tier ${name} --kimi on|off"
      json_py setbool tiers "${name}" kimi "$([[ "$2" == "on" ]] && echo true || echo false)"
      log "tier ${name} kimi = ${2}"; return ;;
    --effort)
...
cmd_doctor() {
  ...
  val="$(json_py get kimiMode)"
  if [[ "${val}" == "cli" ]]; then
    if command -v kimi >/dev/null 2>&1; then log "  ok   kimi CLI: $(command -v kimi) (kimiMode=cli)"
    else log "  FAIL kimiMode=cli but no 'kimi' on PATH — the kimi leaf will be UNAVAILABLE"; rc=1; fi
  fi
...
pi-config.sh — configure the PI review council (~/.claude/pi.json)
  ...
  set kimiMode  <opencode|cli|off>  where the Kimi leaf runs
  ...
  tier  <name> --kimi on|off        move the Kimi leaf
