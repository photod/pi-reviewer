...
model — there is **no cost-gate** here (unlike codex-worker's Sol). Caller MAY pass `tier=med|high|ultra`;
treat it as a ceiling, else classify the task yourself. **Echo the route before delegating**, e.g.
`Routing: high (clearly-scoped bug fix) → <IMPL> @ --variant high`.

| Class you detect | Tier | `--variant` |
|---|---|---|
| Commits, rename, spacing, tiny/small, well-specified 1–2 files | **med** | `low` |
| Normal bug fix / clearly-scoped feature (known files) | **high** | `medium` |
| Complex/unclear work spanning several parts of the repo | **ultra** | `high` (write a TDD plan first, then implement per it) |
...
- **For `ultra`:** first a read-only `--agent plan` call to write a short **TDD plan** (tests, files,
