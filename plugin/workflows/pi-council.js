export const meta = {
  name: 'pi-council',
  description: 'PI code-review council engine (med/high/ultra). Invoked by /pi-review — you normally do not run this directly.',
  phases: [
    { title: 'Review' },
    { title: 'Synthesize' },
  ],
...
// never touches this table. kimi joins at high+ultra (see kimiMode). Synthesis effort tracks STAKES,
// not review count (that's why `high` gets more effort than `med` despite fewer reviews):
// med=routine, high=architecture-adjacent, ultra=pre-release. `effort` (low/medium/high) is used BOTH as
// the Agent-tool effort for the opus/sonnet chairman AND, via variantOf() below, as opencode's `--variant`
// for the cheap chairman — one consistent vocabulary; variantOf normalizes shorthand (med→medium).
const TIERS = {
  med:   { families: ['minimax', 'deepseek', 'mimo'], kimi: false, effort: 'low' },
  high:  { families: ['glm', 'qwen'], kimi: true, effort: 'medium' },
  ultra: { families: ['glm', 'qwen', 'minimax', 'deepseek', 'mimo'], kimi: true, effort: 'high' },
}
...
const rawTier = String(A.tier || 'high').toLowerCase()
const tierName = rawTier === 'max' ? 'high' : rawTier  // 'max' is an accepted alias for 'high'
if (!TIERS[tierName]) throw new Error(`unknown tier '${rawTier}' — use med, high (aka max), or ultra`)
...
// glm-5.2'), bare ('glm-5.2'), or FAMILY ('glm', 'qwen', 'kimi'…). Known caveat: in high/ultra glm is
...
// confabulations, not affirm). Minor, not eliminated (correlated blind spots remain). Drop to 'med' (no
...
// Kimi is CONFIGURABLE via args.kimiMode (applies to kimi-tiers high/ultra only):
...
// Kimi leaf — only in kimi-tiers (high/ultra); backend chosen by kimiMode (opencode/cli/off, above).
...
// reviews; synthesis is a convenience on top. (The ultra dogfood flagged this as the one unguarded SPOF.)
